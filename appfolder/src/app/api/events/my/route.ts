// src/app/api/events/my/route.ts
...
export async function GET() {
  ...
  // Moji RSVP-ani eventi u budućnosti
  const now = new Date().toISOString();

  // 1) event_id gdje sam RSVP-an
  ...
  // 2) sami eventi (budući)
  const { data: evs, error: evErr } = await supabase
    .from("events")
    .select("id,title,sport,description,start_at,end_at")
    .in("id", ids)
    .gte("end_at", now) // samo nadolazeće/tekuće
    .order("start_at", { ascending: true });
  if (evErr) return NextResponse.json({ error: evErr.message }, { status: 500 });

  // 3) primary court info za karticu
